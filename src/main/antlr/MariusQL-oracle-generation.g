 ...
 **********************************************************************************/
package fr.ensma.lias.mariusql.driver.oracle.antlr;
 ...
 */
class OracleGeneratorBase extends TreeParser;
...
options {
	...
	exportVocab=Oracle;    // Call the resulting definitions "Oracle"
...
{
	private static Logger log = LoggerFactory.getLogger(OracleGeneratorBase.class);
...
queryExpression
	...
	| #(EXCEPT { out("("); } queryExpression { out(") minus ("); } queryExpression { out(")"); })
...
query {} 
	...
	#(SELECT {out("select ");}
        ...
        ( #(LIMIT { out(" where rownum "); } selectExprLimit ) )?
        ( #(IMPLEMENTATION {}))?
	)
	;
...
arrayExpr
	: #(a:TARRAYINT { out(a); }  (simpleExprArray | queryExprArray))
	| #(b:TARRAYSTRING { out(b); }  (simpleExprArray | queryExprArray))
	| #(c:TARRAYDOUBLE { out(c); }  (simpleExprArray | queryExprArray))
	| #(d:TARRAYBOOLEAN { out(d); }  (simpleExprArray | queryExprArray))
	;
...
caseExpr
	: #(CASE { out("case"); } 
		...
		( #(ELSE { out(" else "); } expr) )?
		{ out(" end"); } )
	| #(CASE2 { out("case "); } a:expr
		( #(WHEN { out( " when "); } b:expr { out(" then "); } expr) )+ 
		( #(ELSE { out(" else "); } expr) )?
		{ out(" end"); } )
	;
