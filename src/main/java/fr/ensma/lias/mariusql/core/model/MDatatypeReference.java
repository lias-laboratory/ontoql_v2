 ...
 */
public class MDatatypeReference extends AbstractDatatypeReference {
    
    public MDatatypeReference(MariusQLSession pSession, MClass mClass) {
	super(pSession);
	this.setCategory(mClass);
    }
    
    public MDatatypeReference(MariusQLSession pSession, Long pRid, Long pCatId) {
	super(pSession, pRid);
	...
	if (this.category == null) {
		...
		throw new MariusQLException("The category is null");			
	    }
	    this.category = FactoryCore.createExistingMGenericClass(this.getSession(), this.getInternalCategoryId());
	}
	...
	columnsName.add(MariusQLConstants.M_DATATYPE_ON_CLASS_ATTRIBUTE_NAME);
	columnsName.add(MariusQLConstants.M_DATATYPE_IS_SIMPLE_TYPE_ATTRIBUTE_NAME);
